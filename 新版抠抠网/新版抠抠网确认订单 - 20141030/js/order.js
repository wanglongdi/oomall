...
//易货券
$(function(){
   $('.yihuo_check').click(function(){
	   if($(this).is(':checked')){
		   $('.yihuo').find('.yihuo_ma').show();
		}
	   if(!$(this).is(':checked')){
		   $('.yihuo').find('.yihuo_ma').hide();
		}
	})
})

$(function(){
   $('.yihuo_ma .in_btn').click(function(){
	   var val=$(this).siblings('.ma_inp').val();
	   var ma_ul=$('.yihuo_text ul');
	   var ma_li='<li>易货券码:'+val+'面值:<span class="text">'+'50</span>元<span class="close"></span>';
	   if(val=='请输入易货券码'){
		   alert('易货券码不能为空');
		   return;
		}else{
   ...
   $('.yihuo_text .close').live('click',function(){
	   alert(1);
